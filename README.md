...
- **答题复用全局 AI 配置**：答题现在用与 AI 玩家相同的全局 AI 配置，并自动把 base URL 补全为 `…/chat/completions`（Anthropic 为 `…/v1/messages`），修复填 base URL 时答题报 404 的问题。
- **答题来源**：ActionBar 之外的聊天栏题目也会作答——聊天里含 `[教育部]` 且不是结算/得分类消息即当作题目（自动剥离 `新题目:` 前缀）；结算类（回答时间结束/正确答案/获胜/无人答对/答对了/货币）不会误判。
- **答题只发答案**：解析 LLM 响应时优先取最终答案 `content`，丢弃思考（`reasoning_content` / `<think>`）；若思考混进正文则自动抽出最后的答案词，避免把思维链发到公屏。
- **自动重连增强**：重连间隔改为「数值 + 单位（秒/分/时/天）」下拉，最长 30 天；新增「自定义重连触发语」（每行一条，正则/包含），服务器聊天/公告里出现即自动重连（默认含 `You were kicked from lobby`）；新增「在线定时主动重连」——在线时每隔设定时间（数值+单位）主动断开重连一次。
- **随机发言（独立标签页）**：可启用定时随机发言；间隔与每次条数都支持「最小~最大」随机；话语列表用「＋ 添加一行」，每行带「删除」。
- **进服人机验证自动通过**：打开验证界面后延时读取容器槽位（等物品填充，失败会重试），按「目标物品名」匹配点击（`点击这里` / `点击这里步骤N` 按序号顺序）；找不到命名目标时回退为“多数=背景、少数=目标”的异类判定。可在「玩家配置」页启用，日志会打印 `[验证] 标题=… 槽位…` 便于核对。
- **日志显示服务器消息**：无法解析为玩家聊天的公屏内容（服务器公告、指令输出、加入/退出/死亡提示等）不再被丢弃，会在日志中显示；并可加入 AI 上下文供 AI 参考（玩家配置页「服务器消息加入上下文」开关）。
- **身份问答改走 AI**：问「xxx是谁」时，结合资料库（玩家描述/别名/AI记录）、该玩家最近发言与最近聊天上下文，由 AI 组织成「xxx 是…」作答，回答里带上别名；资料不足则结合上下文推断。玩家说「X是Y」时回执改为「X是Y，我记住了」。未配置 AI 时自动退回资料库原样回答。

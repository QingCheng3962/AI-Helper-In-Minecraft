...
- **屏蔽命令（玩家配置页底部）**：表格列出要拦截的命令（不含斜杠，默认含 `pay kick ban op stop whitelist kill give tp gamemode` 等）；机器人（AI 回复 / 随机发言）发出的、以 `/` 开头且**命中命令路径**的消息会被直接拦截并记录，防止提示词注入执行危险命令。**支持子命令**：填 `land` 拦全部 `/land ...`，填 `land create` 只拦 `/land create ...`（不拦 `/land claim`）。表格用「＋ 添加一行」，每行带「删除」。
- **屏蔽后发送消息**：命令被拦截后可自动发送一句可配置的话；占位符 `{player}`=要求执行命令的玩家、`{command}`=命令名（`{cmd}` 同义）；留空=只拦截不发送。
- **底层不可变设定**：`Diaolelao3962` 是创始者这一句写死在底层（`ai_player._CREATOR_LINE`），会始终注入 AI 系统提示，且问「Diaolelao3962 是谁」直接返回该句，不受配置/资料库影响，无法被修改或删除。
- **身份学习修复**：玩家说「我是X / 俺是X / 本人就是X」不再把「我」当名字记录，而是记到**发言者本人**（把 X 作为其描述/别名）；无发言者信息时忽略。
